...
void ARDepth::visualizeImg(const cv::Mat& raw_img, const cv::Mat& raw_depth, const cv::Mat& filtered_depth){
    const int width_visualize = 480;
    const int height_visualize = 270;
...
//        cv::Mat edges = Canny(soft_edges, base_img);
        ...
        // cv::imshow("image", recon.GetImage(frame, false));
        // cv::waitKey(30);
        ...
        cv::resize(last_depth, last_depth_resized, cv::Size(width_visualize, height_visualize), 0, 0, cv::INTER_AREA);

        try {
            cv::imwrite( "./output/"+recon.views[frame].name, last_depth_resized);
        }
